...
export function getChartInfo(nodes: Array<GraphNode>): ChartSizeInfo {
...
export class DagUtil {
  ...
  private linkVertical(s: GraphNode, d: GraphNode, link: GraphLink) {
    const sAnchor = s.family!.anchor
    const dAnchor =
      link.childIndiSpouseId === d.indi?.id
        ? d.indi.anchor
        : link.childIndiSpouseId === d.spouse?.id
        ? d.spouse.anchor
        : null
    if (!sAnchor || !dAnchor) {
  ...
  updateSvgDimensions(chartInfo: ChartSizeInfo) {
    ...
    const transition = this.options.animate ? group.transition().delay(HIDE_TIME_MS).duration(MOVE_TIME_MS) : group
  ...
  renderChart(nodes: Array<GraphNode>, links: Array<GraphLink>): Promise<void> {
    ...
    return Promise.all([nodeAnimation, linkAnimation]) as unknown as Promise<void>
  }

  renderNodes(nodes: Array<GraphNode>, svg: SVGSelection): Promise<void> {
    const formattedNodes: HierarchyPointNode<TreeNode>[] = nodes.map(
      n =>
        ({
          data: {
            id: n.id,
    ...
    const animationPromise = new Promise<void>(resolve => {
      ...
      const updateTransition = this.options.animate
        ? boundNodes.transition().delay(HIDE_TIME_MS).duration(MOVE_TIME_MS).on('end', transitionDone)
        : boundNodes
  ...
  renderLinks(nodes: Array<GraphNode>, links: Array<GraphLink>, svg: SVGSelection): Promise<void> {
    const animationPromise = new Promise<void>(resolve => {
      const link = (parent: GraphNode, child: GraphNode, lk: GraphLink) => {
      ...
      const linkTransition = this.options.animate
        ? boundLinks.transition().delay(HIDE_TIME_MS).duration(MOVE_TIME_MS).on('end', transitionDone)
        : boundLinks
